      ...
      <template slot-scope="{ data }">
        <tbody>
          ...
          <vs-tr v-for="(cv, index) in data" :key="index" :data="cv">
            ...
            <vs-td>
              <vs-chip
                :color="getPublishedStatusColor(cv.is_published)"
                class=""
              >
                {{ cv.is_published ? "Published" : "Unpublished" }}
              </vs-chip>
            </vs-td>
            <vs-td>
              <p class="font-medium">
            ...
            <vs-td class="whitespace-no-wrap">
              ...
              <feather-icon
                v-b-tooltip.hover
                title="Delete Talent and CV"
                icon="TrashIcon"
                svg-classes="w-5 h-5 hover:text-danger stroke-current"
                class="ml-2"
                @click="deleteRecord(cv.id, `${cv.name}`)"
              />
            </vs-td>
          </vs-tr>
        </tbody>
...
export default {
  ...
  methods: {
    ...
    loadRecords() {
      ...
      vm.$store.dispatch("cvs/list", payload).then(() => {
    ...
    deleteRecord(id, name = "") {
      ...
      this.$swal
        .fire({
          ...
          preConfirm: () => {
            return this.$store.dispatch("cvs/delete", id).then(() => {
              if (
    ...
    deleteMultipleRecords() {
      ...
      this.$swal
        .fire({
          ...
          preConfirm: () => {
            return this.$store
              .dispatch("cvs/deleteMultiple", payload)
              .then(() => {
                if (
                  this.$store.state.cvs.success !== false &&
                  this.$store.state.cvs.error === false
