<template>
  ...
  >
    <vs-table
      ...
      :data="campaignsDataSet"
      :total="campaignsDataSetTotal"
      multiple
      search
      :no-data-text="
        loadingCampaignsDataSet
          ...
          >
            ...
            <vs-td class="whitespace-no-wrap">
              <router-link
                v-if="
                  !campaign.is_managed ||
                    (campaign.is_managed && !campaign.is_published)
                "
                :to="{ name: 'edit-campaign', params: { id: campaign.id } }"
                tag="a"
    ...
    <vs-pagination
      v-show="campaignsDataSetLastPage > 1"
      v-model="dataTableFilter.page"
      :total="campaignsDataSetLastPage"
      :max="9"
      :goto="true"
    ></vs-pagination>
  </div>
...
export default {
  ...
  computed: {
    ...mapState("users", {
      loadingCampaignsDataSet: state => state.loadingCampaignsDataSet,
      error: state => state.error,
      success: state => state.success,
      validationErrors: state => state.validationErrors,
      campaignsDataSet: state => state.campaignsDataSet,
      campaignsDataSetTotal: state => state.campaignsDataSetTotal,
      campaignsDataSetLastPage: state => state.campaignsDataSetLastPage
    }),
    ...
    dataTableStats() {
      ...
      let toRecord =
        this.campaignsDataSetTotal -
          this.currentPage * this.dataTableFilter.per_page >
        0
          ? this.currentPage * this.dataTableFilter.per_page
          : this.campaignsDataSetTotal;
      let ofTotal = this.campaignsDataSetTotal;
